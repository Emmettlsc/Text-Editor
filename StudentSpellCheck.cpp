...
bool StudentSpellCheck::load(std::string dictionaryFile) {  //O(N)
    root = new TrieNode;
...
void StudentSpellCheck::getsuggestions(std::string word, int max_suggestions, std::vector<std::string>& suggestions){
    int i=0;
...
void StudentSpellCheck::spellCheckLine(const std::string& line, std::vector<SpellCheck::Position>& problems) {  //O(oldP + S + W*L)
...
void StudentSpellCheck::insertwordintotrie (TrieNode* root, std::string insert){
...
void StudentSpellCheck::initializeTrieNode (TrieNode* target){
    for(int i=0; i<27; i++){
...
bool StudentSpellCheck::searchTrie(std::string check){  //returns true if found
...
void StudentSpellCheck::freeNodes(TrieNode* killmychildren){
