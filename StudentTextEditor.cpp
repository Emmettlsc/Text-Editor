...
StudentTextEditor::StudentTextEditor(Undo* undo)    //O(1)
 : TextEditor(undo) {
        mrow = mcol = 0;
...
StudentTextEditor::~StudentTextEditor()     //O(N)
{
    mlines.clear();
}

bool StudentTextEditor::load(std::string file) { //O(M+N+U)
...
bool StudentTextEditor::save(std::string file) {    // O(M) M=num of char in editor
...
void StudentTextEditor::reset() { // O(N+U) U=num items in undo stack
    mrow = mcol = 0;
...
void StudentTextEditor::move(Dir dir) {  // O(1)
...
void StudentTextEditor::del() { // O(L)
...
void StudentTextEditor::backspace() {       // O(L) or O(L1 + L2)
...
void StudentTextEditor::insert(char ch) {   //needs O(L) where L is length of line of text
    if(ch != '\t') {
...
void StudentTextEditor::enter() {     // O(L) L= length of line of text
    std::string s1, s2;
    if(mcol==0)
        s1 = "";
    else
...
void StudentTextEditor::getPos(int& row, int& col) const {  // O(1)
    row = mrow; col = mcol;
}
