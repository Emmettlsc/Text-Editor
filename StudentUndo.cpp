...
StudentUndo::Action StudentUndo::get(int& row, int& col, int& count, std::string& text) {   //O(1)
    if(UndoStack.empty())   //returns if empty
        return Action::ERROR;
    row = UndoStack.top().mrow;
    col = UndoStack.top().mcol;
    text = "";
    if(UndoStack.top().mAction==Action::INSERT) //only changes count if studenteditor needs to delete chars
